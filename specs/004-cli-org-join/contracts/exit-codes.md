...
| ---: | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- |
| `12` | `ORG_REQUIRED`         | Authenticated but the caller has no Organization Membership. Emitted by `frugl upload` only — `GET /api/orgs/me` returned `409 org_required` and upload cannot proceed without a destination. `frugl whoami` does NOT use this code (it reports the no-org state and exits 0). | FR-028; US5; spec edge case "`GET /api/orgs/me` returns `409 org_required` during `upload`" |
...
| `41` | `ENDPOINT_UNREACHABLE` | An explicit `--endpoint` / `FRUGL_ENDPOINT` host unreachable from the first attempt (spec edge case "Network unreachable").                                                                               |
