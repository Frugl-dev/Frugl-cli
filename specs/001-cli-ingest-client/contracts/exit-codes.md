...
| `41` | `ENDPOINT_UNREACHABLE`  | Explicit `--endpoint` (or `FRUGL_ENDPOINT`) host was unreachable from the very first attempt — distinct from a flaky default endpoint. | Spec edge case "User passes `--endpoint` pointing at an unreachable host".                                                                               |
