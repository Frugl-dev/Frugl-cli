...
| **III. Astro + React Islands + Supabase Auth**                          | partial          | Supabase Auth remains the only identity provider: the handoff code is a _delivery mechanism_ for an ordinary Supabase browser session, created server-side in the Astro app; the service-role key stays server-side; the CLI still does not embed `@supabase/supabase-js` (001 research R-8 upheld). ✅ **Pass**                                                                                                                                                                                                              |
...
R-7) — is an _honest_ degradation, not a silent one: the reason is surfaced, and the
