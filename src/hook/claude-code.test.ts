...
import { installHook, uninstallHook, isInstalled, settingsPath } from "./claude-code.js";
...
describe("claude-code hook install/uninstall", () => {
  it("installs a SessionEnd hook running the headless upload (project scope)", () => {
    const cwd = makeTmp();
    const { path: file } = installHook("project", { cwd });
    expect(file).toBe(settingsPath("project", { cwd }));
  ...
  it("uninstall removes only the managed entry and leaves others intact", () => {
    ...
    installHook("project", { cwd });
    const result = uninstallHook("project", { cwd });
    ...
    expect(isInstalled("project", { cwd })).toBe(false);
  });
  ...
  it("fails loud on malformed settings JSON (refuses to clobber)", () => {
    ...
    expect(() => installHook("project", { cwd })).toThrow(UsageError);
